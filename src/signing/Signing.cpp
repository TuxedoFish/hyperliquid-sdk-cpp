    ...
    const std::optional<uint64_t>& expiresAfter)
{
    ...
    if (isAuthenticated(type))
    {
        ...
        {
            auto action = body["action"];
            // agentSendAsset and createVault are unusual L1 actions: the exchange requires the
            // action's own nonce field to equal the envelope nonce below, and checks it strictly
            // (server rejects the request as a whole with a deserialize error if it's missing),
            // unlike every other L1 action here which carries no embedded nonce at all.
            if (type == RestEndpointType::AgentSendAsset || type == RestEndpointType::CreateVault)
                action["nonce"] = nonce;
