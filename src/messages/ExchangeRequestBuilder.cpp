...
namespace hyperliquid
{
    void ExchangeRequestBuilder::initializeMapping(const ApiConfig& config, RestApi* api)
    {
        ...
        {
            int assetId = pairIndex + 10000;
            bool isAutoGeneratedName = !pair.name.empty() && pair.name[0] == '@';
            symbolMap_.add(isAutoGeneratedName ? ("@" + std::to_string(pairIndex)) : pair.name, assetId);
            if (pair.isCanonical && !isAutoGeneratedName && pair.tokens.size() == 2)
            {
                int baseTokenIndex = pair.tokens[0];
                for (const auto& token : spotMetaResponse.meta.tokens)
                {
                    if (token.index == baseTokenIndex)
                    {
                        symbolMap_.add(token.name, assetId);
                        break;
                    }
                }
            }
