...
4. **`include/hyperliquid/rest/RestApiMessageParser.h`** + **`src/rest/RestApiMessageParser.cpp`** - add a `parseX` method that turns the raw JSON response into your response struct. Start the `try` block with `validateStructure(message);` (see every existing `parseX` method) before touching the document - this is what makes malformed/adversarial input a clean caught exception instead of a process abort (see issue #109).
...
- **Looking up more than one field by name on the same object can abort the process, not just throw, on malformed input** - simdjson's ondemand `obj["key"]` lookup has an internal assertion that fires (not a catchable error) when the search has to skip past other fields on a structurally malformed object. Every `parseX` method's `validateStructure(message)` call (see above) guards against this centrally, so as long as you follow the standard boilerplate you get this for free - just don't skip that line. See issue #109 for the full story.
